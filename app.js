document.addEventListener('DOMContentLoaded', () => {
  ...
  // Estado general
  const getJornadaLogica = () => {
    const ahora = new Date();
    const horaCorte = 6; // El día laboral cambia a las 6 AM
    ...
    container.innerHTML = state.puestos
      ...
      <div class="puesto" style="border-left: 5px solid ${getColorPuesto(p)}">
        ...
        <div class="tarea-buttons">${config.ordenTareas
          .map(
            (t) => `<button class="add-tarea-btn ${config.abrev[t] || t}" data-puesto="${p}" data-tarea="${t}" aria-label="Añadir tarea ${config.abrev[t] || t} al puesto ${p}">${config.abrev[t] || t}</button>`
          )
          .join('')}</div>
      </div>`
      )
  ...
  function renderDashboard() {
    ...
    let table =
      ...
      config.ordenTareas.map((t) => `<th>${config.abrev[t] || t}</th>`).join('') +
  ...
  function renderLog() {
    document.getElementById('log-container').innerHTML = state.log
      ...
      .map(
        (l) => `
      <div class="log-entry">
        <span><strong style="color:${getColorPuesto(l.puesto)};">Puesto ${l.puesto}</strong> | ${l.hora} | ${config.abrev[l.tarea] || l.tarea}</span>
    ...
    cont.innerHTML = fechas
      .map((f) => {
        ...
        return (
          ...
          logAgrupado[f]
            .map((l) => `<div class="log-entry"><span><strong style="color:${getColorPuesto(l.puesto)};">Puesto ${l.puesto}</strong> - ${l.hora} - ${config.abrev[l.tarea] || l.tarea}</span><button class="eliminar-log-historial-btn" data-id="${l.id}" aria-label="Eliminar registro">🗑️</button></div>`)
            .join('') +
          '</div>'
        );
      })
  ...
  function finalizarJornada() {
    ...
    state.jornadaActual = getJornadaLogica();
    saveJornada();
